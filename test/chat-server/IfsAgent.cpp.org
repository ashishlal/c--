        ...
        string& org, string& common_name )
{
    ...
    netsnmp_ds_set_string(NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_X_SOCKET, m_agentx_loc.c_str());
...
void IfsAgent::mainloop_nonblocking(const boost::system::error_code& error)
{
    ...
    // ATP_LOG_INFO("IfsAgent", "-----mainloop:1----- ");
...
int IfsAgent::snmpTrap(int level, string & errorMsg)
{
    ...
    }
    {
        ...
        switch (arg = snmp_parse_args(argc, (char **)(m_argv), &session, "C:", optProc)) {
        ...
        }
        {
            ...
            if (snmp_add_var(pdu, objid_snmptrap, sizeof(objid_snmptrap) / sizeof(oid),'o', m_argv[arg]) != 0) {
                snmp_perror(m_argv[arg]);
                SOCK_CLEANUP;
                goto freeres;
                exit(1);
            }
        ...
        while (arg < argc) {
            ...
            if (!snmp_parse_oid(m_argv[arg - 3], name, &name_length)) {
                snmp_perror(m_argv[arg - 3]);
                SOCK_CLEANUP;
                goto freeres;
                exit(1);
            }
            // int snmp_add_var (netsnmp_pdu *pdu, const oid *name, size_t name_length, char type, const char *value)
            if (snmp_add_var(pdu, name, name_length, m_argv[arg - 2][0],m_argv[arg - 1]) != 0) {
                snmp_perror(m_argv[arg - 3]);
                SOCK_CLEANUP;
                goto freeres;
                exit(1);
                               ...
                               netsnmp_request_info *requests)
{
    ...
    IfsAgent *s = (IfsAgent *)agent;
    if(s->get_disable_snmp()) {
        return SNMPERR_GENERR;
    }
    sprintf(stats_parameter_value, s->get_common_stat((unsigned int)(stats_parameter)).c_str(), 
                                ...
                                netsnmp_request_info *requests)
{
    ...
    stats_parameter_value = (unsigned int)(s->get_stat((unsigned int)(stats_parameter)+STAT_END_COMMON_STATS));
                           ...
                           netsnmp_request_info *requests)
{
    ...
    stats_parameter_value = (unsigned int)(s->get_stat((unsigned int)(stats_parameter)+STAT_END_MIXER_STATS));
